...
public class GLGizmos : MonoBehaviour
{
    ...
    private static void InternalDraw2DBoxArray(Vector2 center, float width, float height, Vector2 arrayDimensions, bool solid, Color[,] colors = null)
    {
        ...
        //if (colors != null && (arrayDimensions.x != colors.GetLength(0) || arrayDimensions.y != colors.GetLength(1)))
        //{
        //    Debug.LogError("GLGizmos [Draw2DBoxArray]: Color array dimenions do not match arrayDimensions");
        //    return;
        //}
        ...
        if (collider is BoxCollider2D)
        {
            ...
            if (boxCollider.transform.rotation.eulerAngles.z == 0)
            {
                ...
                if (boxCollider.edgeRadius > 0)
                    InternalDrawBoxEdgeRadius((Vector2)boxCollider.transform.position + boxCollider.offset, boxCollider.size.ScaleEach(boxCollider.transform.lossyScale.Abs()), boxCollider.edgeRadius);
            }
            else
            {
                ...
                if (boxCollider.edgeRadius > 0)
                    InternalDrawBoxEdgeRadiusRotated((Vector2)boxCollider.transform.position + boxCollider.offset, boxCollider.size.ScaleEach(boxCollider.transform.lossyScale.Abs()), boxCollider.edgeRadius, boxCollider.transform.rotation.eulerAngles.z);
            }
        }
